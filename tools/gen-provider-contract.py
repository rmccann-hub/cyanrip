...
def emit(binary):
    ...
    w("- `-v`, `-V` and `--version` all print the version banner and exit 0.")
    w("  Upstream moved this flag from `-V` to `-v` when it replaced getopt with")
    w("  genopt after 0.9.3; a caller probing with `-V` against a stock 0.9.4 build")
    w("  gets exit 1 and `Unable to parse command line argument: -V`, which reads")
    w("  as \"not installed\" rather than \"flag renamed\". This fork accepts `-V`")
    w("  again. **Prefer `--version`** -- it has never changed and never will.")
