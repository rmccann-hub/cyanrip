...
def sc_golden_reference_is_from_a_clean_build():
    ...
    if not banner_sha:
        ...
        #
        # TWO CHECKS, NOT ONE, AND THE SECOND EXISTS BECAUSE THE FIRST IS
        # SATISFIED BY THE DOCUMENT BEING WRONG. `sha in text` asks only "did
        # anyone type these seven characters", while the thing that decays is
        # the PREDICATE beside them. Measured 2026-09-18: every genuine mention
        # of the banner build was removed from `Changelog.md` and replaced with
        # "The golden reference was NOT generated by <sha>; that commit is
        # unrelated to it" -- and this check PASSED. It was satisfied by a
        # sentence denying the exact claim it exists to verify.
        #
        # Found by running the grep Platterpus handed us in round 21 lap 4 §H3,
        # where the same shape had bitten their standing-status currency gate:
        # *a currency gate that matches on an identifier rather than on the
        # claim about it is satisfied by the very sentence it was written to
        # catch.* Their remedy cites our own round-6 rule back at us -- where a
        # check matches on a label, make it also require the subject; the label
        # answers "did they name it", the content answers "did they write it",
        # and only the pair is a check.
        #
        # BOTH ARE KEPT, for the reason they kept both of theirs: the first
        # catches a build no document mentions at all, the second catches one a
        # document mentions while claiming something else about it. Neither
        # subsumes the other.
        ...
        if not named:
            ...
            # The claim, not the token. A window rather than the single line,
            # because the pairing is a sentence and prose wraps: the failure
            # this guards is a stale or contradicting predicate, not a line
            # break. `committed at` is required too, because that is the half
            # this test can never verify itself -- naming it is the author's
            # undertaking and the message has always demanded it.
            claimed = False
            for home in homes:
                if not home.exists():
                    continue
                ...
                for i, line in enumerate(lines):
                    ...
                    # BOTH SPELLINGS OF THE SECOND HALF. `CLAUDE.md`'s canonical
                    # form is "generated by X, committed at Y", but the entry
                    # already in `Changelog.md` for the previous regeneration
                    # says "committed IN the commit carrying this entry" -- and
                    # it is making exactly the claim this asserts. A check that
                    # accepts one spelling of a correct statement and rejects
                    # the other is the near-miss pattern defect pointing the
                    # wrong way: it would force the prose to change to suit the
                    # test rather than assert a property. The property is that
                    # the author named where it LANDED as well as what BUILT
                    # it; "at" and "in" both do that, and neither weakens the
                    # pairing, because `generated by` is still required beside
                    # it. Found 2026-09-18 when the round-21 close needed a new
                    # entry and the neighbouring one would not have passed.
                    landed = "committed at" in window or "committed in" in window
                    if "generated by" in window and landed:
                        claimed = True
                        break
                if claimed:
                    break
            if not claimed:
                fail(f"{sha} is named somewhere, but no document CLAIMS it -- "
                     "the golden reference's build must appear in a "
                     "'generated by X, committed at Y' sentence, not merely as "
                     "a string. A mention that says anything else, or nothing, "
                     "satisfies the check above and means nothing")
