...
- **A grep pattern that nearly matches is worse than one that does not.** The
  ...
  **AND A SECOND INSTANCE SURVIVED IN A NEIGHBOURING TEST FOR WEEKS AFTER THAT
  RULE WAS WRITTEN, which is the part worth keeping.**
  `sc_golden_reference_is_from_a_clean_build()` asserted `sha in
  p.read_text()` over every round file and `Changelog.md` — *did anyone type
  these seven characters* — while its own comment claimed the stronger
  property, *"the generating build must be named, and 'generated by X,
  committed at Y' must both appear"*, and its failure message demanded both.
  **The comment, the message and the check were three different specifications
  and only the weakest one ran.**

  Measured 2026-09-18: every genuine mention of the banner build was removed
  and replaced with *"The golden reference was NOT generated by `6061202`; that
  commit is unrelated to it"* — **and the check passed.** Satisfied by a
  sentence denying the one claim it exists to verify.

  **It was found by a peer's grep, not by ours.** Platterpus reported the shape
  in round 21 lap 4 §H3 after it bit their own standing-status currency gate —
  *a currency gate that matches on an identifier rather than on the claim about
  it is satisfied by the very sentence it was written to catch, one value
  later* — and offered it at the could-in-any-possible-way bar with *"one grep
  on your side settles it either way"*. **That is an instruction to compare, not
  to acknowledge.** Running it took one command and found a live defect; the
  tell they named is exact: **a gate whose assertion is `identifier in text`
  while the thing that decays is the PREDICATE beside the identifier.**

  Two things follow. **Grep for the shape, not for the instance** — a rule
  written from one test does not fix its siblings, and nothing in this
  repository was looking. And when a check, its comment and its failure message
  disagree about what is being asserted, **the check is the only one that
  runs**, so the other two are documentation of an intention nobody executed.
