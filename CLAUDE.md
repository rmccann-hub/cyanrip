...
- `src/pregap.c`/`.h` -- Q sub-channel pregap search for physical discs, carried
  from upstream PR #115 with fixes; see `PREGAP-DETECTION-CARRY.md`.
- `tests/` -- meson-driven unit tests (`fun512.c`, `naming.c`, `subq.c`) plus
...
`master` is a clean mirror of upstream `cyanreg/cyanrip`; `platterpus-fork` is
the only development branch. Nothing else should hold commits.
