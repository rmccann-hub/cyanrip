...
HANDSHAKE-TESTED: **the real test on the pair, on a drive, and the run did not pass**, 258 of 261, all three failures section F's rip killed from outside both programs. Bundle sha256 `f14864171bdbb215e555e777d51fc8dcbd466b4306d56a9441d42640d01a4571`, filed here at `3d6954b` and in your tree at `platterpus@6c1890b:docs/handshake/artifactsround26/`, 47 blobs, among which all 40 of our files' blob ids appear. Both readings are done: ours in lap 4, with §B's correction, and yours in your lap 5 over all eight of your reports. **Not tested, by either side:** section F's whole-disc fast path, and a sector that will not read. Ours besides: the full suite at `cd56d2c`, 89 of 89, from a removed log with one run header and 89 result lines. Since then the tree has changed only in documents.
HANDSHAKE-FROM-COMMIT: dca47a3
HANDSHAKE-FROM-COMMIT-SOURCE: the parent of the commit that adds this lap. It is reachable from `platterpus-fork`, and every `file:line` of ours below resolves there: `src/` has not changed since `ed4a377`.
