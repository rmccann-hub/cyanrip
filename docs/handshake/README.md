...
| 23 | **CLOSED `GO`/`GO`**, 2026-09-22, five laps by the measure `CLAUDE.md` states — the highest `HANDSHAKE-LAP` either side declared; four by Platterpus's own count — 30 days before the declared close-by. Opened the same day, the day after round 22 closed. **It reports hardware rather than demanding it** — the full acceptance session ran first, on 2026-09-22, 247 of 247 steps on `2cce60d` + Platterpus 0.6.52, filed at `docs/rig-2026-09-22-2cce60d/`; that is round 21's lesson applied, whose five laps came of naming a condition no desk lap could supply. Three close conditions, fixed at lap 1 and unable to grow (R1/S-13): **§0.1** `PROTOCOL.md` v5 — the close rule plus Platterpus's condition on it, that a lap count only once it declares `HANDSHAKE-READY-TO-READ: yes`; **§0.2** that a HELD lap's draft verdict reaches the compiled `Handshake:` line, which is a change to that line's value vocabulary and so a condition rather than a commit; **§0.3** that every non-pass in the acceptance run is dispositioned — fixed, filed with a named owner, or shown not to have been real — which is deliberately not *"zero failures"*, a finish line neither side controls. **We pre-commit to `GO`.** The run reached the two round-21 items `fe4d2c4` could not: `Retry limit:` on real logs, and `Ripping errors:` as the moved field on a real interrupted session. Two findings, neither a condition — tracks 3 and 5 were superseded by an automatic re-rip with no addendum written (**wrong, and retracted in lap 3; see below**), and every audio figure our log reports is measured upstream of the filter graph. `+platterpus.14` is **explicitly a non-condition**: it ships on round 22's authority, after Platterpus's both-wordings release. **Lap 2 is theirs and answers all three**: §0.2 assented after running four banner shapes through their real parser, §0.3 agreed on four of five rows, §0.1 assented in substance with the drafting left to us — plus a `partial` grade on their own script, because three of eight rips had post-rip checks dropped unfinished and no step could see it. **Their §A refutes our §0.3 row 2 and they are right**: `retried_tracks` records `replaced: false` on tracks 3 and 5, a swap happens only on a converged re-read, so nothing was superseded and the absent addendum was a correct negative. Retracted in lap 3 §A; the cause was filing the session without the `.platterpus.json` records and then reasoning about a question one field in them answers. **Lap 3 is ours and is `GO`**, discharging lap 1's pre-commitment, and it lands `PROTOCOL.md` **v5** — §5b the close rule, §5c their condition on it, `HANDSHAKE-PEER-VERDICT-SOURCE` their field, rows C37–C42. Implemented but inert: every path is gated on the FILE's declared version and nothing declares 5, so **round 23 still costs the extra lap v5 exists to remove**. Their gate reached 5 later that day, at `platterpus@c2f43d28` — and their question about §5b's *"enumerated"* then showed that, as written, step 3 cannot fire on a real record (`docs/KNOWN-ISSUES.md`), so v5's saving waits on v6 wording. **Lap 4 is theirs**: v5 committed byte-identical, `GO`/`GO`, and their gate closes it — but ours could not, because §0.1's condition names `seam-sync-check --fetch` and it still exited **1**: v5 was on their working branch and not on `main`, the default branch the tool reads and the one their own `FROM-COMMIT-SOURCE` calls *"the ref you can fetch"*. Held open one exchange rather than closed on a check that said no. **Lap 5 is ours and is the close**, once `main` carried v5 at `platterpus@48776b0` and the tool exited 0 on all four. **And §D2 happened**: the PR squash-merged and the branch was deleted four minutes later, orphaning both commits this round's citations name — one of them in a lap already sent. Recovered from a session clone before GitHub ran `gc`, which is luck and is filed as an event rather than a success. Lap 5 adopts their round-24 proposal early: **the sha256 is the anchor, the commit is a fetch hint** | *(pin **`2cce60d`**, unchanged from round 22 and it never moved (S-15) — that round reviewed this commit's contract, this one reviews its behaviour on a drive; **no test pin**, declared `none`)* | `round-23-lap-05.md` |
| 24 | **CLOSED `GO`/`GO`**, 2026-09-23, **three laps**, 13 days before the declared close-by. Opened 2026-09-22, the day round 23 closed and `+platterpus.14` shipped. **One close condition, on purpose**: Platterpus's verdict on `3e01bb3`, from their parser reading a log that commit wrote. It existed to end a window the operator opened knowingly: `.14` went to **stable** by the operator's instruction, over the plan's recommended beta, so Platterpus's app offered it stamped `unapproved` until their `FORK_PIN` rolled — and lap 1 §0 said so. Declared protocol 5 on both sides. **Our lap 1 declared `GO`**, so their lap 2 (`GO`, their 0.6.53 parser reading our golden reference with zero unrecognised lines, every footer arm covered by substitution) closed it on their gate, and our lap 3 closed it on ours. **Three laps is the lap-1 `GO`, not v5**: lap 1 §B3 found that §5b as written cannot save a lap, and their gate closed one lap earlier than ours on the decision-time reading, their round-25 item N1. Their `FORK_PIN` rolled to `3e01bb3` at `platterpus@86f0547`; users get it in 0.6.54. Lap 3 §F records the operator's decision on their N4: a strict release gate for `v0.*`. Round 25 takes the rest, chosen from their compiled agenda | *(pin **`3e01bb3`** = `+platterpus.14`, moved at the round boundary from `2cce60d` and never during the round; **no test pin**, declared `none`)* | `round-24-lap-03.md` |
| 25 | **CLOSED `GO`/`GO`**, 2026-09-23, **five laps**, 14 days before the close-by. Opened by our lap 1 on text: `PROTOCOL.md` v6, `OWNERSHIP.md` v3 and `seam-rules.md` v6 byte-identical in both trees. **Our lap 2 carries the operator's instructions of the same day**, in the operator's words: as few rounds as needed; fix as much as we can; physical CD rips, not arguing over bugs and language; and **every round ends on usable releases of both applications**, with the real test on the released pair opening the next round. It added that as close condition §0.3 by recorded override of R1, and v6 makes it permanent as **R8** and **R9**. Before lap 2 we fixed rather than filed, including **a hang**: `-r 3` never returned on one unreadable sector, because libcdio-paranoia checks the retry limit only at multiples of 5 (`2af669e`). **Their lap 2 crossed ours** and answered lap 1 only, which cost one lap. Our lap 3 proposed the one v6 both trees could hold, with R8 point 1 corrected because their `FORK_PIN` must be a closed round's pin. Their lap 4 (`GO`) landed it, parsed our golden reference and named 0.6.54. Our lap 5 (`GO`) landed it here, set our gate to 6, and closed the round. Next, under R8: `.15`, then their 0.6.54 pinning `3e01bb3`, then the real test, which opens round 26 | *(pin **`3e01bb3`**, never moved (S-15); **no test pin**)* | `round-25-lap-05.md` |
| 26 | **CLOSED `GO`/`GO`**, 2026-09-24, **six laps**, 27 days before the close-by. Opened 2026-09-23 by our lap 1 on `df91ae7`, the released `+platterpus.15`. **It opens before the real test, by the operator's override of v6 R8 point 3.** The reason is that Platterpus's acceptance run requires the installed ripper to be their `PIN_UNDER_REVIEW`, which tracks the newest `HANDSHAKE-PIN` we send, so the test can run on `.15` only after a lap names it. **Close conditions:** the real test on `.15`, installed through their app; each side's reading of the bundle; and R8's two releases, their `FORK_PIN` roll to `df91ae7` and our `.16`. Hardware is a close condition under R8 point 4's exception, because the round reviews a build on a drive. **Their lap 2** (`OPEN`) moved `PIN_UNDER_REVIEW` to `df91ae7` in 0.6.54. **Their lap 3** (`OPEN`) says 0.6.54's acceptance run refused `.15`, and names **0.6.55** as the fix. **The real test ran on 0.6.55 on 2026-09-24** and did not pass, 258 of 261, all three failures one container kill (`docs/rig-2026-09-24-df91ae7/`), which Platterpus's round 27 lap 2 traced to a container held by an earlier Platterpus window's unit; what struck it is not identified. **Our lap 4** (`GO`, released 2026-09-24) reads cyanrip's logs in it and names `.16`. **Their lap 5** (`GO`) closes it on their gate and rolls their `FORK_PIN` to `df91ae7`. **Our lap 6** (`GO`, released 2026-09-24) records it and closes it on ours, with one correction of lap 4 | *(pin **`df91ae7`**, set at the boundary (S-15); **no test pin**, because the pin is a released build)* | `round-26-lap-06.md` |
| 27 | **CLOSED `GO`/`GO`**, 2026-09-26, **six laps**, 26 days before the close-by. Opened 2026-09-24 by our lap 1 (marked released at `87facd5`, returned to held under the operator's override, revised to name Platterpus 0.6.59, and released again) on `221a1df`, the released `+platterpus.16`, before the real test, by the operator's override of R8 point 3, as round 26 was. **Close conditions:** the Full acceptance on `.16` installed through their app, from a release whose `PIN_UNDER_REVIEW` is `221a1df` (their 0.6.58 is not: its step A accepts only `df91ae7`); each side's reading of the bundle; and R8's releases, their `FORK_PIN` roll to `221a1df` and our `.17`. Carried, not conditions: the `Accurip 450` wording, album loudness, their three bug patterns, and proposed wording for R8 point 3. **Their lap 2** (`OPEN`) moved `PIN_UNDER_REVIEW` to `221a1df` in **0.6.59** (`183073b`). **Their lap 3** (`OPEN`) says 0.6.59's first Full attempt stopped at section A with `.15` installed, and names **0.6.60** (`88c09dd`) as the fix. **The operator then chose to close the round without a Full or Standard run**, and to test both projects' next releases together in round 28. **Our lap 4** (`GO`, released 2026-09-26) records that as an override of R1, stands the 2026-09-26 quick run (`docs/rig-2026-09-26-221a1df-quick/`) in for §0.1, and names `.17`. **Their lap 5** (`GO`) accepts the override and both rewordings, closes it on their gate and rolls their `FORK_PIN` to `221a1df`. **Our lap 6** (`GO`, released 2026-09-26) records it and closes it on ours. It corrects lap 4's candidate, which now carries `ee0221c` as well, and it is the first lap whose body is in LSL (`PROPOSAL-lap-statement-language.md`). **The Full run lap 1 §0.1 asked for then ran after all**, from 04:13:08Z the same day: its script reported 320 of 320, and Platterpus's ledger grades it `partial` by the operator's ruling (`docs/rig-2026-09-26-221a1df/`); lap 6, written during it, says otherwise, and `STATUS.md` corrects it | *(pin **`221a1df`**, set at the boundary (S-15); **no test pin**)* | `round-27-lap-06.md` |
| 28 | **OPEN**, opened 2026-09-26 by our lap 1 on `e0471f4`, the released `+platterpus.17`, before the real test, by the operator's override of R8 point 3, as rounds 26 and 27 were. **Close conditions:** the Full acceptance on `.17` installed through their app, from their 0.6.61 with `PIN_UNDER_REVIEW` `e0471f4`; each side's reading of the bundle; and R8's releases, their `FORK_PIN` roll to `e0471f4` and our `.18`. Our lap 1 is in LSL, and answers their LSL amendments 1 by id | *(pin **`e0471f4`**, set at the boundary (S-15); **no test pin**)* | `round-28-lap-01.md` |
...
prerequisite. **Platterpus releases 0.6.61 after our round 28 lap 1 names it**,
carrying `FORK_PIN` `221a1df` and `PIN_UNDER_REVIEW` `.17` together, so their app
offers `.17` marked `unapproved` until round 28 reviews it. Round 28 opens on
