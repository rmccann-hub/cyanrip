...
| 25 | **OPEN**, opened 2026-09-23 by our lap 1. **Our lap 2 carries the operator's instructions of the same day**, given after lap 1 was released, in the operator's words: as few rounds as needed; fix as much as we can; physical CD rips, not arguing over bugs and language; and **every round ends on usable releases of both applications**, with the real test on the released pair opening the next round and its bundle in both repositories. **Close conditions:** lap 1's two, the three texts byte-identical in both trees (`PROTOCOL.md` v6, `OWNERSHIP.md` v3, `seam-rules.md` v6, starting texts in `docs/handshake/proposed/`), plus lap 2's **§0.3**, both releases ready and agreed, added by a recorded operator override of R1. The proposed v6 makes the instruction permanent as **R8** (a round ends on two releases) and **R9** (fix it, do not argue it). Before lap 2 we fixed rather than filed: round 23's `Handshake:` draft qualifier (`20a5aca`); v6's close rules in our gate, dormant until v6 lands (`82a6154`); and **a hang**, `-r 3` never returning on one unreadable sector because libcdio-paranoia checks the retry limit only at multiples of 5 (`2af669e`). That one was found with a fault-injection shim and no drive, and Platterpus's rig was left at 3. **Their lap 2 crossed ours**: released the same day, `GO`, it answers our lap 1, holds no lap 2 of ours, and lands our lap 1 v6 with one amendment, so their tree has no R8 or R9. OWNERSHIP v3 and seam-rules v6 are now landed in both trees (ours at `c07bf68`). **Our lap 3** proposes the one v6 both trees can hold (`05abdfde…`: lap 2's text, their amendment verbatim, and R8 point 1 corrected, because their `FORK_PIN` must be a closed round's pin and so cannot name `.15`), and asks for the two §0.3 items their lap 2 did not carry: their parser over our golden reference, and their release candidate. **Pre-committed:** our lap 5 lands the protocol, sets our gate to 6, carries the ledger and is `GO` unless their lap 4 changes a row's outcome, refuses a clause, or cannot read our golden reference. Then `.15` releases, then theirs | *(pin **`3e01bb3`**, unchanged (S-15); `.15` candidate **`61711f1`**; **no test pin**)* | `round-25-lap-03.md` |
...
**Round 25 is open, so no release is permitted** — `tools/release-gate.py
--release-gate` exits 1 and names it. The live release is
...
errors.` and the new `Encoder errors:` line are P2. **Round 24 reviewed `.14`
and closed in three laps.** Their `FORK_PIN` rolled to `3e01bb3` on their `main`
at `platterpus@86f0547`, and users get it in 0.6.54. Until then their 0.6.53
still approves `2cce60d` and offers `.14` stamped `unapproved`, the window the
operator accepted when `.14` went to stable. With round 25 open, 0.6.54 also
waits under the strict gate the operator chose for their `v0.*` line, unless it
goes out under a written override.

**`proposed/` holds shared-document texts under review, not the spec.** Round 25
lap 1 proposed `PROTOCOL.md` v6, `OWNERSHIP.md` v3 and `seam-rules.md` v6 there,
by sha256. The last two have landed and been removed from it (`c07bf68`). What is
left is `PROTOCOL-v6.md`, the text round 25 lap 3 proposes. Until it lands, the
file at `docs/handshake/PROTOCOL.md` is the one this tree holds, and Platterpus's
`main` holds a different v6.
