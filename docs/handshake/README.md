...
| 25 | **OPEN**, opened 2026-09-23 by our lap 1, released the same day. **Two close conditions, both text**, fixed at lap 1 and unable to grow (R1/S-13): **§0.1** `PROTOCOL.md` v6 and **§0.2** `OWNERSHIP.md` v3 with `seam-rules.md` v6, byte-identical in both trees. The starting texts are in `docs/handshake/proposed/`, each built from the current file by asserted edits. v6 carries what round 24 agreed for it: K1–K3, §5b step 1 and C37 on the decision-time reading, the two false §8 sentences deleted, the agreed-change ledger, and what a citation names. It also carries three corrections, marked as not on that list: C43, C13a amended, and `ACK` retired. Our gate fixes for their D1 and D2 landed first, at `cc235a1` and `12a85fd`. The second found six `GO` laps after a close in our record, which is the evidence for amending C13a. Lap 1 §E places every other agenda item by name. **Pre-committed:** our next lap lands the texts as their `main` carries them and is `GO`, unless they change a row's expected outcome or refuse a clause. Opening now also holds their 0.6.54 under the strict `v0.*` gate, unless it is overridden, and lap 1 §D says so | *(pin **`3e01bb3`**, unchanged from round 24 and not moving (S-15); **no test pin**, declared `none`)* | `round-25-lap-01.md` |
