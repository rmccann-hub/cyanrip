...
**Round 26 is open**, so `tools/release-gate.py --release-gate` exits 1 and names
it. It reviews `.15` on a drive, and it opened before the real test, on the
operator's override, so that Platterpus's app installs `.15` for it. The live release is **`0.9.4-rc2+platterpus.15` at `df91ae7`**, `release_seq` 25,
...
with two fixes from that test, once round 26 closes on our gate.
