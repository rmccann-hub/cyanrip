...
| 22 | **OPEN**, opened 2026-09-18 on the new pin, held at lap 1. Carries the **operator's goal for the round** — fix all known issues, release both applications by the close, then a full hardware acceptance test to **open round 23**. Two close conditions and **neither needs a drive**: §0.1 the two shared-protocol changes (their K1, our K3), §0.2 whether a Platterpus release can be cut inside this round. Declares `HANDSHAKE-PROTOCOL: 4` and proposes 5, because both sides must ship a version before either declares it | *(pin **`2cce60d`** = `+platterpus.13`, released on round 21's authority; **no test pin**, declared `none` as an answer)* | `round-22-lap-01.md` |
