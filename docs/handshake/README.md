...
| 21 | **OPEN** — and closing. Laps 1–3 sent; §0.2 closed by their refusal. **§0.1 is answered by the SECOND 2026-09-17 session**, on the test pin: item 1 established, item 3 established with its limit stated, and **item 2's wording was never satisfiable** — `Retry limit:` is in their `_IGNORED_DISC_LINES` by design, so we wrote a close condition asserting a mechanism in their code without reading it. The risk it guarded is retired. **Their lap 4 is released** (2026-09-18, their `main` at `5ea3d2c`, `VERDICT: GO`) and filed here byte-exact; our **lap 5 is HELD** with `VERDICT: GO` and `PEER-VERDICT: GO`. The gate names one remaining reason — lap 5 is not released — so the close waits on one operator act | *(pin `fe4d2c4` frozen; **test pin `3952c03`** is what the session finally ran, `docs/rig-2026-09-17-3952c03/`; the first session of that date ran `fe4d2c4` and is void, `docs/rig-2026-09-17-fe4d2c4/`)* | `round-21-lap-05.md` |
