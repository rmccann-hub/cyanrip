...
> ```
> **The SHA is in the manifest and deliberately not in this file.** A lap cannot
> name a build that contains it: the handshake state is compiled in, so this
> file changes the binary, and the binary that ships with it is always the
> commit after. `ddf7ac3` stays stable and retained, so downgrade is possible.
>
...
> commit. Both are named in this lap's §I once the release lands, and
...
`PROVIDER-CONTRACT.md` ships with this release, regenerated from a clean build,
`--check` exits 0, and the generating commit is named in the release commit's
message. **No content delta beyond the version string**: no `cyanrip_log()` call
