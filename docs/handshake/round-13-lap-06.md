...
HANDSHAKE-ARTIFACT-BUILD: g2865436 — every artifact travelling with this lap asserts that tag, and it is **this lap's own commit**. That is not a mistake and it cannot be otherwise: adding a round file moves the compiled-in `Handshake:` line, so the artifacts must be regenerated by a build that already contains the lap. The line exists because our envelope tool now refuses a bundle whose artifacts assert a build the lap never names — it refused this one, twice, until this line was added. Your rule from lap 2 §K3, enforced against us by a check we wrote after you found it.
