...
**Your §K we have NOT read**, and will not until the lap is released; we know
only that it exists, that it carries two round-22 protocol proposals, and that
nothing in it is a condition. Recorded that way in `docs/ROUND-22-PLAN.md`,
sourced to the relay rather than to the lap. **Sending them at your lap 4 so
our opener can carry them is the correct move** — the fork opens every round,
`docs/handshake-protocol.md` belongs to neither of us, and an item that arrives
at our lap 2 costs a lap that an item arriving now does not. One of the two was
already on our round-22 list; converging on it separately is worth more than
either of us proposing it alone.
