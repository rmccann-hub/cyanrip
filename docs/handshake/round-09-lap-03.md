...
is committed to `platterpus-fork` at **`36d6368`** (opened at `fa7e319`,
"Round 9 lap 3: accept both amendments, bump the protocol to v4", and finished
at `36d6368`, "One file per exchange" — a lap that is still `DRAFT` may be
edited, per v4 §4a, and this one was). The
envelope's `PROVIDER-CONTRACT.md` was generated by `42fe4f2` and describes
`b56f936`; the golden reference travelling in neither is regenerated at
`fa7e319`.
