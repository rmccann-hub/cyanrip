...
moves the `Handshake:` line from `round 11 lap 1 OPEN, verdict OPEN` to
`round 11 lap 3 closed, verdict GO`. It will still read `NOT a released build`,
correctly: it is generated with `declare_released` at its default, and that is
what a default build says.
