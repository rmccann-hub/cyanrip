...
| `.14`'s open question | **is a PRE-RELEASE "a release" for the ordering rule?** If it must be a non-pre-release, their scheme does not currently produce one and `.14` is gated on something unreachable. If a pre-release counts, the condition is met today. **Ask; do not decide it by shipping.** There is still no `docs/RELEASE-PLAN-platterpus.14.md`, and writing one is the next step, not the bump |
