...
STATUS-NEWEST-LAP: round-27-lap-04.md
...
| gate | `--release-gate` exits **1** and names round 27, which is open. That is correct: `.16` is released on round 26's authority, and round 27 closes only when Platterpus's lap 5 declares `GO` |
| the tip | **the release's publish commit and after.** Build from the release commit, not the tip |
| next | **Platterpus's round 27 lap 5.** Our lap 4 is `GO` by the operator's override of §0.1: no Full or Standard run in round 27, the 2026-09-26 quick run in its place, and one Full run in round 28 on both projects' next releases together. If their lap 5 is `GO`, our lap 6 transcribes it, then `.17` ships, our round 28 lap 1 names it, and their one release carries `FORK_PIN` `221a1df` and `PIN_UNDER_REVIEW` `.17` |
