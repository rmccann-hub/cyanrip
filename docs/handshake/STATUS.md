...
| next | **the Full acceptance on Platterpus 0.6.60 with `.16` installed.** `v0.6.60` is `88c09dd`, where `PIN_UNDER_REVIEW` is `221a1df`, so its section A accepts `.16`, and neither its update offer nor its setup wizard replaces it. The bundle goes into both repositories, then our lap 4 reads it |
