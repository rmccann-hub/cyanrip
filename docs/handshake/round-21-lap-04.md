...
file changes the binary — the compiled `Handshake:` banner moved from `round 21
lap 3` to `round 21 lap 4` — so the golden reference was regenerated:
**generated by `3c1ab33`, committed alongside this paragraph.** Never the same
commit and it cannot be. The audio and every checksum over it are unchanged; what
differs is the build SHA, the `Handshake:` line, and the timing and wall-clock
fields that differ between any two runs of the same binary.
`PROVIDER-CONTRACT.md` is unaffected and its `--check` exits 0, since nothing in
`src/` changed.
