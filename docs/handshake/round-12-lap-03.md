...
HANDSHAKE-PIN-POLICY: UNMOVED, and that is the whole point of this line. S-15 froze it at lap 1 and neither side asked for it back. Six commits have landed since -- §D -- and not one is in the reviewed pin. It is still not a release: no `release-ledger.tsv` row names it, `release-manifest.json` still resolves both channels to `c4d1a00`, and every log a build of it writes says `NOT a released build`.
