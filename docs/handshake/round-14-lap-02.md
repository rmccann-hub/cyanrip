...
| `docs/round-14-acceptance-spec.md` | what we expect the pass to observe, per test |
...
> **Generated by the commit that adds this lap, and committed at the commit
> immediately after it**, which this file cannot name because it cannot carry the
> hash of the commit containing it. Adding a round file changes the compiled-in
> handshake state, so these could not exist until this lap did.
