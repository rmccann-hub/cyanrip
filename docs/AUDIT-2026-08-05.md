...
>
> A third finding came out of shipping them: **`PROVIDER-CONTRACT.md` could not
> see §4.1.** P2 derives each entry from the *format string* at the call site,
> and §4.1 changes an argument, so the row is byte-identical across the change
> and `--check` exits 0. The contract derives a line's shape, not the meaning of
> its numbers. Not patched — a half-working argument extractor would put wrong
> text into a document presenting itself as derived. Proposed for round 8.
