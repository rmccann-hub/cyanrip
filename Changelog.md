...
Unreleased
==========
 - **Round 15 lap 14 sent** — the golden reference is **generated by
   `39aa928`** (the lap-14 commit) and committed at the commit after it, which
   is the only order possible: an artifact cannot contain the hash of the build
   that produced it. **Their acceptance run arrived, completed and passed** —
   ...
   **The run was produced by Platterpus `0.6.37` at `f3b60a0`, not the `0.6.38`
   their lap 13 declared**, and by the older acceptance script — none of the
   three verbs their §C1 says `0.6.38` introduces appear, while the two their
   §A1 calls satisfiable by finding nothing do. Reported as lap 14 §H; CC-1
   stays theirs to assess under S-14. Why the older build ran is recorded as
