...
Unreleased
==========
 - **Round 15 lap 5 sent** — CC-1 met by Platterpus's 2026-09-03 pass on the
   released pin, the P5 defect that pass found declared `HANDSHAKE-BREAKING`,
   and the peer half declared `unknown` because the rig ran `0.6.34` and
   nothing we hold names its commit. References regenerated behind it,
   **generated by `475f427`** (the lap-5 commit), committed at the commit after
   it. Same ordering as every lap: commit, **rebuild**, regenerate.

   Lap 5 speaks out of turn on purpose. Lap 3 assigned lap 4 to them, and lap 3
   also carries a `HANDSHAKE-TESTED` the bundle falsified; leaving that standing
   while they verify against it is worse than a crossed lap. Their lap 4 keeps
   its number.
