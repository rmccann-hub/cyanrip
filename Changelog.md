...
Unreleased
==========
 - **Round 15 opened at lap 1** on the released pair, and the references were
   regenerated behind it — **generated by `b5cdbb8`** (the commit adding the
   lap) **and committed at the commit after it**. Adding a lap file moves the
   compiled-in handshake state, so `Handshake:` changes in every log.
