...
`t * CACHE_HIT_RATIO < miss_cost`. Eight filed rig sessions report *"at least
2048 sectors … search ceiling reached"* and **not one recorded the dozen timings
behind it**, so all eight are unusable for fixing the defect they demonstrate —
...
**ROUND 21 IS OPEN, AND THE TEST PIN IS `2c3deff`.** Two close conditions, fixed
