...
and the provider contract's source anchor is unmoved.
 ...
 - **Round 14 laps 7, 9, 11, 13 and 14**, and their inbound counterparts. Lap 13
   ...
   > **The provider contract is generated by `12d77c1`** — the commit that
   > reshapes the `Cache model:` fix — and **the golden reference by `d90e73c`**,
   > the commit that adds lap 15; each is committed at the commit after the one
   > that built it — **and committed at the commit after it**, because a generated
   > artifact cannot carry the hash of the build that produced it. Its
   > predecessors were generated by `a2589d1` and `b27f3b8`; all three are named
   > because a build a log can claim must stay resolvable. Only the compiled-in
   > handshake state moved each time: `round 14 lap 9`, then `lap 11`, `lap 13`,
   > `lap 14`.
