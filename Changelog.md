...
`0.9.4-rc2+platterpus.8` and one build tag between them. The build that
generated them is named in the release commit's own message; a changelog entry
cannot name the commit that contains it, for the same reason a handshake lap
cannot carry the hash of the tree containing it. An earlier draft of this line
did name a SHA, computed before an `--amend` that changed it, leaving a
dangling reference in the very paragraph whose subject is dangling references. Those are always two commits and always will be — and here the
reason is sharper than usual: adding a round file changes the compiled-in
handshake state, so the lap that opens a round cannot name the build its own
artifacts were generated by. That is why this paragraph is in the changelog
rather than in the lap.
