...
and the provider contract's source anchor is unmoved.
 ...
 - **Round 14 laps 7, 9, 11 and 13**, and their inbound counterparts. Lap 13
   ...
   > **The golden reference is generated by `a2589d1`** — the commit that adds
   > lap 13 — **and committed at the commit after it**, because a generated
   > artifact cannot carry the hash of the build that produced it. The previous
   > one was generated by `b27f3b8` and committed at `516cb72`; both are named
   > because a build a log can claim must stay resolvable. Only the compiled-in
   > handshake state moved each time: `round 14 lap 9`, then `lap 11`, then
   > `lap 13`.
