Unreleased
==========
**Handshake round 13 is open** and nothing here is released. Every default build
of this tree writes `NOT a released build` into every logfile, and
`release-manifest.json` still resolves both channels to `237a4ff`.

The golden reference and the interrupted sample shipped in this tree were
**generated by `a494aa7`**, and committed by the commit that adds this line --
which cannot name itself, for the same reason a handshake lap cannot carry the
hash of the tree containing it. An earlier draft of this line did name a SHA,
computed before an `--amend` that changed it, leaving a dangling reference in
the very paragraph whose subject is dangling references. Name the build; the
committing commit is the one the reader is already looking at. (They were previously
generated by `673a57b` and committed at `40aaa82`; that pair is what round 13
lap 1 shipped, and lap 3 §E corrects what the lap said about it.) Those are always two commits and always will be — and here the
